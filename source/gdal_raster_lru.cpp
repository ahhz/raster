...
blink::raster::LRU::block::block(int row, int col, std::size_t size, optional_block_list_iterator& vector_element, GDALRasterBand* band)
  : m_major_row(row)
  , m_major_col(col)
...
blink::raster::LRU::block_cache::block_cache(size_t capacity) : m_capacity(capacity), m_size(0)
{}

blink::raster::LRU::block_cache::block_cache(blink::raster::LRU::block_cache&& that)
  ...
  GDALRasterBand* band)
{
  ...
  //! is the global cache
blink::raster::LRU::block_manager::block_manager(GDALRasterBand* band, block_cache* cache)
...
//!
const void* blink::raster::LRU::block_manager::get(size_t index) const
{
  block_list_iterator i = get_list_iterator(index);
  return i->get_buffer();
}
...
blink::raster::LRU::block_manager::block_list_iterator 
  blink::raster::LRU::block_manager::get_list_iterator(size_t index) const
{
